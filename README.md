...
The current use of this model doesn't leverage the AR-net properties due to the better generalizability (and results) of the neural net for this particular task (yielding better performance results - See the Neural_Network Folder), and more substantially, its need for flexibility (adding an auto regressive component makes the model more rigid). NeuralProphet overall is also ideal for use in this particular case for a number of other benefits it offers, such as its extensive variety in hyper-paramter tuning abilities, powerful PyTorch backend, robustness to missing data (uncontrollable due to fluctuation in Snapchat reporting delays), and suitability toward true future forecasting (See Neural_Network Folder). The NeuralProphet framework makes it very straightforward to calculate predictions for future dates, whereas the same is not so straightforward when considering and testing other popular options for time-series predictive modelling (such as LSTM for example).
