...
def forecast_totalview(choose_episode, choose_hours):
  ...
  #Get Test CTR (If applicable)
  #episode_df = channel_df[channel_df['story_id'].isin([choose_episode])]
  ...
  #Enough hours to forecast vs not continued...
...
def momentum_chart(choose_episode):
  ...
  for index, row in daily_df.iterrows():
    #channel_avg = round(channel_alleps['topsnap_daily_diff'].mean())
...
def summary_table():
  ...
  for story in df.story_id.unique():
    ...
    #Last actual if there isn't enough data for forecasting OR there is long delays in data reporteing
    ...
    #Metrics if there is long delays in data (>72 hours - typical when channels have very low performance and don't report views)
    elif hours_diff > 72:
    ...
    #CTR
    #ctr_df = benchmarks[benchmarks.story_id.isin([story])]
    #if len(ctr_df) >0:
    ...
    if len(historical) < 10 or len(channel_df.loc[channel_df['forecast_type'] == 'future']) <=0 or hours_diff > 72:
      ...
      if len(channel_alleps['topsnap_daily_diff'].dropna()) == 0:
        ...
        #daily_avg = round(channel_alleps['topsnap_daily_diff'].mean())
  ...
  final_df['Consideration'] = np.select(
    [   #Let It Ride
        #No shaba; 48 hours and trending 75% or above and dailyt perf > daily avg * 0.6 (includes double and triple colds)
        ...
        #Not Shaba; 48 hours and avg perf > daily avg * 1.5 (includes double and triple fire)
        |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
        &(final_df['Fcst Period']==48)
        &(final_df['Fcst % vs Bench']>0.5)
        &((final_df['Daily Performance'])>=(final_df['Daily Avg']*1.5))

        #Any; at 72 hours and trending 75% or above AND daily performance >= avg performance*0.8
        |(final_df['Fcst Period']==72)
        &(final_df['Fcst % vs Bench']>=0.75)
        &((final_df['Daily Performance'])>=(final_df['Daily Avg']*0.8))
        #Any; from 72 hours with double trend and  +50% or above; 72-96 hrs with triple hot trend
        |(final_df['Fcst Period']==72)
        &(final_df['Fcst % vs Bench']>=0.5)
        ...
        #Any; at 96 hours and trending 100% or above AND daily performance >= avg performance*0.9
        |(final_df['Fcst Period']==96)
        &(final_df['Fcst % vs Bench']>=1.0)
        &((final_df['Daily Performance'])>=(final_df['Daily Avg']*0.9))
        #Any; at 96 hours and trending aboce 50% with triple fire
        |(final_df['Fcst Period']==96)
        &(final_df['Fcst % vs Bench']>0.5)
        &(final_df['Trend Sentiment']== '🔥🔥🔥')

        #Any; between 120 and 168 trending 100% + and daily performance >= daily avg
        |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
        &(final_df['Fcst % vs Bench']>=1.0)
        &((final_df['Daily Performance'])>=(final_df['Daily Avg']))
        &(final_df['Trend Sentiment']!= '🥶')
        #Any; 120 to 168 where daily performance is 100% greater than daily avg
        |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
        &((((final_df['Daily Performance']) - (final_df['Daily Avg'])) / (final_df['Daily Avg'])) >= 1.0)
        #Any 120-168; triple hot trend
        |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
        &(final_df['Trend Sentiment']== '🔥🔥🔥')

        #Any between 168 and 192 hours and trending 150% or greater + daily performance > daily avg
        |(final_df['Fcst Period']>168) & (final_df['Fcst Period']<=192)
        &(final_df['Fcst % vs Bench']>=1.5)
        &((final_df['Daily Performance'])>(final_df['Daily Avg']))

        #Any at 192 trending 200% or greater + daily performance
        |(final_df['Fcst Period']==192)
        &(final_df['Fcst % vs Bench']>=2.0)
        &((final_df['Daily Performance'])>(final_df['Daily Avg']))

        #Any between 216 and 240 + daily performance
        |(final_df['Fcst Period']>=216) & (final_df['Fcst Period']<=240)
        &(final_df['Fcst % vs Bench']>=3.0)
        &((final_df['Daily Performance'])>(final_df['Daily Avg']))
        #Any > 240 and above 400% (5X an average episode)
        ...
        #Any at 48 hours, trending below -50% and daily perf < avg
        (final_df['Fcst Period']== 48)&(final_df['Fcst % vs Bench']<= -0.5)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']))
        #Any at 48 hours, trending below -75%
        |(final_df['Fcst Period']== 48)&(final_df['Fcst % vs Bench']<= -0.75)
        #Any at 48 hours, trending below 0% where daily perf < avg *0.8
        |(final_df['Fcst Period']== 48)&(final_df['Fcst % vs Bench'] < 0)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']*0.8))

        #AT 72 trending below 0%
        |(final_df['Fcst Period']== 72)&(final_df['Fcst % vs Bench']< 0)
        #AT 72 trending 25% below daily avg
        |(final_df['Fcst Period']== 72)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']*0.75))
         #AT 72 trending below 25% where daily perf < daily avg *0.9 (10% below avg)
        |(final_df['Fcst Period']== 72)&(final_df['Fcst % vs Bench']< 0.25)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']*0.9))

        #Any; at 96 and trending below 50% AND daily performance < daily avg *1.5
        |(final_df['Fcst Period']== 96)&(final_df['Fcst % vs Bench']< 0.5)
        &((final_df['Daily Performance']) < (final_df['Daily Avg']*1.5))
        #Any; at 96 with triple cold trend
        |(final_df['Fcst Period']== 96)
        &(final_df['Trend Sentiment']== '🥶🥶🥶')
        #Any; at 96 where daily performance < daily avg *0.9
        |(final_df['Fcst Period']== 96)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']*0.9))
        #Any; at 96 trending below 75% and daily performance < daily avg
        |(final_df['Fcst Period']== 96)&(final_df['Fcst % vs Bench']< 0.75)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']))

        #Any; 120 to 168 trending less than 75%
        |(final_df['Fcst Period']>= 120)&(final_df['Fcst Period']<= 168)
        &(final_df['Fcst % vs Bench']<= 0.75)
        #Any; 120 to 168 with double or triple cold trend
        |(final_df['Fcst Period']>= 120)&(final_df['Fcst Period']<= 168)
        &(final_df['Trend Sentiment']== '🥶🥶')
        |(final_df['Fcst Period']>= 120)&(final_df['Fcst Period']<= 168)
        &(final_df['Trend Sentiment']== '🥶🥶🥶')
        #Any; 120 to 168 where daily performance is less than daily avg
        |(final_df['Fcst Period']>= 120)&(final_df['Fcst Period']<= 192)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']))

          #Any; 192 , trend is less than 200%
        |(final_df['Fcst Period'] == 192)
        &(final_df['Fcst % vs Bench'] <2.0)
        #Any; 192 and daily performance is less than daily avg
        |(final_df['Fcst Period'] == 192)
        &((final_df['Daily Performance'])<(final_df['Daily Avg']))

        #Any; 216-240 and trend is less than 300%
        |(final_df['Fcst Period'] >= 216) &(final_df['Fcst Period'] <= 240)
        &(final_df['Fcst % vs Bench'] < 3.0)
        #Any 216 or greater but daily performance < daily avg
        |(final_df['Fcst Period'] >= 216)
        &((final_df['Daily Performance']) < (final_df['Daily Avg']))
        #Any; 264 or greater and trend is less than 400%
      ...
      #Not Shaba; 48 hours trending >50% and sentiment is single fire
      (~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
      &(final_df['Fcst Period']==48)
      &(final_df['Fcst % vs Bench']>=0.5)
      &(final_df['Trend Sentiment']== '🔥')
      #Not Shaba; 48 hours trending 0-50% where daily perf > daily avg *1.5 (includes double and triple fire trends)
      |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
      &(final_df['Fcst Period']==48)
      &(final_df['Fcst % vs Bench']> 0) &(final_df['Fcst % vs Bench']< 0.5)
      &((final_df['Daily Performance'])>=(final_df['Daily Avg']*1.5))
      #Not Shaba; 48 hours trending 25-50% and momentum is 50% or greater
      |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
      &(final_df['Fcst Period']==48)
      &(final_df['Fcst % vs Bench']>=0.25) & (final_df['Fcst % vs Bench']<=0.5)
      &(final_df['Momentum %'] >= 0.5)
      #Not Shaba; 48 hours trending 0-25% and momentum is 75% or greater
      |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
      &(final_df['Fcst Period']==48)
      &(final_df['Fcst % vs Bench']>0) & (final_df['Fcst % vs Bench']<=0.25)
      &(final_df['Momentum %'] >= 0.75)

      #Any; at 72 trending between 50% and 75% and daily performance > daily avg *1.75
      |(final_df['Fcst Period']==72)
      &(final_df['Fcst % vs Bench']>=0.5) & (final_df['Fcst % vs Bench']<=0.75)
      &((final_df['Daily Performance'])>=(final_df['Daily Avg']*1.75))
      #Any; at 72 trending between 50% and 75% with single hot trend
      |(final_df['Fcst Period']==72)
      &(final_df['Fcst % vs Bench']>= 0.5) & (final_df['Fcst % vs Bench']<0.75)
      &(final_df['Trend Sentiment']== '🔥')
      #Any; at 72 trending between 0% and 50% with double hot trend
      |(final_df['Fcst Period']==72)
      &(final_df['Fcst % vs Bench']> 0) & (final_df['Fcst % vs Bench']<=0.5)
      &(final_df['Trend Sentiment']== '🔥🔥')

      #Any; 96 trending between 75-100% with single or double fire trend
      |(final_df['Fcst Period'] == 96)
      ...
      &(final_df['Fcst % vs Bench']>= 0.75) &(final_df['Fcst % vs Bench']<= 1.0)
      &(final_df['Trend Sentiment']== '🔥')
      #Any; at 96 trending between 75% and 100% with single hot trend or positive momentum
      |(final_df['Fcst Period']==96)
      &(final_df['Fcst % vs Bench']>= 0.75) & (final_df['Fcst % vs Bench']< 1.0)
      &(final_df['Trend Sentiment']== '🔥')
      |(final_df['Fcst Period']==96)
      &(final_df['Fcst % vs Bench']>= 0.75) & (final_df['Fcst % vs Bench']< 1.0)
      &(final_df['Momentum %'] > 0)
      #Any; at 96 trending greater than 25% with daily perf >= daily avg *1.5 (includes double and triple hot)
      |(final_df['Fcst Period']==96)
      ...
      &(final_df['Fcst % vs Bench']>0.25)
      &((final_df['Daily Performance'])>=(final_df['Daily Avg']*1.5))
      #120-168 between 75-100% ""
      |(final_df['Fcst Period']>=120) &(final_df['Fcst Period']<=168)
      &(final_df['Fcst % vs Bench']>=0.75) & (final_df['Fcst % vs Bench']<1.0)
      &((((final_df['Daily Performance']) - (final_df['Daily Avg'])) / (final_df['Daily Avg'])) >= 0.5)

      #Any; at 120 to 168 trending between 75% and 100% with single or double hot trend
      |(final_df['Fcst Period']>=120) &(final_df['Fcst Period']<=168)
      &(final_df['Fcst % vs Bench']>=0.75) & (final_df['Fcst % vs Bench']<1.0)
      &(final_df['Trend Sentiment']== '🔥')
      |(final_df['Fcst Period']>=120) &(final_df['Fcst Period']<=168)
      &(final_df['Fcst % vs Bench']>=0.75) & (final_df['Fcst % vs Bench']<1.0)
      &(final_df['Trend Sentiment']== '🔥🔥')
      #Any at 192 and trending between 150-200% but daily perf > daily avg * 1.5
      |(final_df['Fcst Period']==192)
     ...
     #Investigate - Bearish
     #Not Shaba; 48 hours trending 0% to -75% and daily perf < daily avg * 1.25
     (~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period']==48)
     &(final_df['Fcst % vs Bench']< 0) & (final_df['Fcst % vs Bench']> -0.75)
     &((final_df['Daily Performance']) < (final_df['Daily Avg']*1.25))
     #Not Shaba; 48 hours trending -25% to 75% with single cold trend or daily performance < daily avg *0.8
     |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period']==48)
     ...
     &(final_df['Fcst % vs Bench']< 0.75) & (final_df['Fcst % vs Bench']>= -0.25)
     &((final_df['Daily Performance'])<= (final_df['Daily Avg']*0.8))
     #Not Shaba; 48hrs trending >75% where daily perf < daily avg *0.6
     |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period']==48)
     &(final_df['Fcst % vs Bench']>= 0.75)
     &((final_df['Daily Performance'])<= (final_df['Daily Avg']*0.6))
     #Not Shaba; 48 hours with double or triple cold trend
     |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period']==48)
     &(final_df['Trend Sentiment']== '🥶🥶')
     |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     ...
     #Any; at 72 trending between 25 and -25%
     |(final_df['Fcst Period']==72)
     &(final_df['Fcst % vs Bench']< 0.25) & (final_df['Fcst % vs Bench']>= -0.25)
     #Any; 72 where fcst between 0 and +50 and daily performance < avg performance
     |(final_df['Fcst Period']==72)
     &(final_df['Fcst % vs Bench']> 0) &(final_df['Fcst % vs Bench']< 0.5)
     &((final_df['Daily Performance'])<(final_df['Daily Avg']))
     #Any; at 72 trending where trend is double trend
     |(final_df['Fcst Period']==72)
     &(final_df['Trend Sentiment']== '🥶🥶')
     #Any; 72 where  daily performance < avg performance *0.75
     |(final_df['Fcst Period']==72)
     &((final_df['Daily Performance'])<(final_df['Daily Avg']*0.75))
     #Any; between 72 and 96 with single cold trend and daily performance < avg performance
     |(final_df['Fcst Period']>=72) &(final_df['Fcst Period']<=96)
      &(final_df['Trend Sentiment']== '🥶')
      &((final_df['Daily Performance'])<(final_df['Daily Avg']))

      #Any; 96 where daily performance < avg performance
     |(final_df['Fcst Period']==96)
     &((final_df['Daily Performance'])<(final_df['Daily Avg']))
     #Any; 96 between 25% and 75% with single cold trend
     |(final_df['Fcst Period']==96)
     ...
     #Any between 120 and 168 hours, where trend is single cold
     |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
     &(final_df['Trend Sentiment']== '🥶')
     #Any between 120 and 168 hours, trending greater than 100% but daily performance < daily avg
     |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
     &(final_df['Fcst % vs Bench']>= 1.0)
     &((final_df['Daily Performance'])<(final_df['Daily Avg']))
     #Any between 120 and 168 hours, trending between 75 and 100% but momentum is negative
     |(final_df['Fcst Period']>=120) & (final_df['Fcst Period']<=168)
     &(final_df['Fcst % vs Bench']>= 0.75) &(final_df['Fcst % vs Bench']<= 1.0)
     &(final_df['Momentum %'] <= 0)

      #Any between 168 and 192 hours and trending >150% but momentum < -25%
      |(final_df['Fcst Period']>168) & (final_df['Fcst Period']<=192)
      &(final_df['Fcst % vs Bench'] >= 1.5)
     ...
     #Investigate - Average
     #Non-Shaba; at 48 hours trending anywhere from 0% to +75%
     (~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period'] == 48)
     &(final_df['Fcst % vs Bench'] >= 0) &(final_df['Fcst % vs Bench'] <= 0.75)
     #Non-Shaba; at 48 hours trending between -25% and 0% but daily perf > daily avg *1.25
     |(~final_df['Channel'].isin(['What The Fork!?', 'Snacks & Hacks', 'The Shaba Kitchen', 'The Pun Guys']))
     &(final_df['Fcst Period'] == 48)
     &(final_df['Fcst % vs Bench'] >= -0.25) &(final_df['Fcst % vs Bench'] < 0)
     &((final_df['Daily Performance'])>=(final_df['Daily Avg']*1.25))

     #Any; at 72 trending anywhere from 25 and +75%
     |(final_df['Fcst Period']==72)
     &(final_df['Fcst % vs Bench'] >= 0.25)&(final_df['Fcst % vs Bench'] < 0.75)

     #Any; at 96 trending between 50-100%
     |(final_df['Fcst Period']==96)
     &(final_df['Fcst % vs Bench'] >= 0.5)&(final_df['Fcst % vs Bench'] < 1.0)

     #Any; 120 to 168  trending 75% to 100%
     |(final_df['Fcst Period']>=120)&(final_df['Fcst Period']<=168)
     &(final_df['Fcst % vs Bench']>= 0.75) & (final_df['Fcst % vs Bench']< 1.0)
...
def evaluate_model():
  eval = ('''SELECT story_id_fixed as story_id,
              ...
              FROM ML.EVALUATE(MODEL`plaid-creator-portal.dynamic_scheduling.arima_dynamic_scheduling`) arima
              LEFT JOIN (SELECT account_id,
                                post_id,
                                account_name,
                                title,
                                datetime(published_at, "America/Toronto") AS published_at
                          FROM `plaid-creator-portal.Views.snap_publisher_profile`
                          ) AS pub
              ON arima.story_id_fixed = pub.post_id
              WHERE pub.account_name NOT IN ('Hacksmith', 'Ray Reacts', 'That Was Epic', 'Future Of AI', 'Dr. Karan')
...
def get_data():
  arima_query_test = ('''
    WITH cte AS
      ...
      FROM ML.FORECAST(MODEL `plaid-creator-portal.dynamic_scheduling.arima_dynamic_scheduling`,
      ...
      SELECT story_id_fixed story_id,
        ...
        FROM (SELECT *
              FROM ML.EXPLAIN_FORECAST(MODEL `plaid-creator-portal.dynamic_scheduling.arima_historical`,
    ...
    LEFT JOIN (SELECT post_id,
                      ...
                      title,
                      published_at
                FROM `plaid-creator-portal.ingest.post_details`) AS pub
    ...
    LEFT JOIN (SELECT datetime(DATE_TRUNC(interval_time, HOUR)) AS interval_rounded,
                  ...
                  FROM `plaid-creator-portal.Views.snap_posts_rt`
                  WHERE TRUE
    ...
    WHERE pub.account_name NOT IN ('Hacksmith', 'Ray Reacts', 'That Was Epic', 'Future Of AI', 'Dr. Karan', 'Drinks Drinks Drinks')
    ORDER BY story_id, interval_time ASC
    )
    SELECT *
    FROM cte_2
    ...
    FROM
          ...
          FROM `plaid-creator-portal.Views.snap_posts_rt` AS v
          WHERE TRUE
          AND EXTRACT(MINUTE FROM interval_time) = 0
          AND v.post_id in (SELECT story_id_fixed
                            FROM `plaid-creator-portal.dynamic_scheduling.dynamic_scheduling_episodes`
                            WHERE story_id_fixed IS NOT NULL
                            GROUP BY story_id_fixed)
          AND v.post_id NOT IN (SELECT story_id
                                  FROM cte_2
                                  GROUP BY story_id)
          AND v.publisher_name NOT IN ('Hacksmith', 'Ray Reacts', 'That Was Epic', 'Future Of AI', 'Dr. Karan', 'Drinks Drinks Drinks')
          )realtime
    ORDER BY story_id, interval_time ASC;
  ''')
